...
"""
Updated Jan 2024 by Tim Murdoch (timothy.j.murdoch@gmail.com) to allow non-image TCSPC data to be read.
...
#                 print(tagName, tagValue)
    ...
    def get_tcspc_decay(self):
        ...
        decay_data = get_tcspc_decay_static(sync, tcspc, channel)
...
@njit
def get_tcspc_decay_static(sync, tcspc, channel):
    
    num_chan     = np.max(channel)+1
    num_tcspc_channel    = np.max(tcspc)+1
    ...
    decay_data = np.zeros((num_tcspc_channel, num_chan))#, dtype  = np.uint16)
    
    for event in range(tcspc.size):
        tcspc_bin = tcspc[event]
        channel_num = channel[event]
        decay_data[tcspc_bin, channel_num] += 1
...
@njit
def get_fsc_data(sync, tcspc, channel):
